...
def train(config, train_loader, model, criterion, optimizer, epoch,
    ...
    for i, (input, target, target_weight, meta) in enumerate(train_loader):
        ...
        if isinstance(outputs, list):
            loss = criterion(outputs[-1], target, target_weight)
            # loss = 0
            for j in range(len(outputs)-1):
                # loss += criterion(outputs[j], outputs[-1], target_weight)
                loss += 0.5*criterion(outputs[j], outputs[-1], target_weight)+0.5*criterion(outputs[j], target, target_weight)
            output = outputs[0]
        # if isinstance(outputs, list):
        #     loss = 0.5*criterion(outputs[0], target, target_weight)+0.5*criterion(outputs[0], outputs[-1], target_weight)
        #     for output in outputs[1:-1]:
        #         loss += 0.5*criterion(output, target, target_weight)+0.5*criterion(output, outputs[-1], target_weight)
        #     loss = 0.5*criterion(outputs[-1], target, target_weight)
        #     output = outputs[-1]
