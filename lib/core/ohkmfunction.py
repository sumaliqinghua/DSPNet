...
def train(config, train_loader, model, criterion,criterion_normal,criterion_normal_2,criterion_hard,optimizer, epoch,
    ...
    for i, (input, target, target_weight, meta) in enumerate(train_loader):
        ...
        if isinstance(outputs, list):
            ...
            loss += criterion_normal(outputs[1], target, target_weight) #【see】要不就写成列表传入
            loss += criterion_normal_2(outputs[2], target, target_weight) #【see】要不就写成列表传入
            loss += criterion_hard(outputs[3], target, target_weight) #【see】要不就写成列表传入
            output = outputs[2]#给vali用的
        else:
            output = outputs
