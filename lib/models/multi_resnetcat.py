...
class MultiResNet(nn.Module):
    ...
    def __init__(self, block, layers, cfg, **kwargs):  # **kwargs代表什么含义
        ...
        self.change_channel_final =nn.Sequential(
            nn.Conv2d(
            in_channels=768,  # 【】把deconv改为resnet对应的通道数
            out_channels=256,
            kernel_size=1,  # 1
            stride=1,
            padding=0
        ...
        self.channel_att2 = SEBlock(256,8)
        self.channel_att3 = SEBlock(256,8)
        self.channel_att4 = SEBlock(256,8)
        self.channel_att5 = SEBlock(256,8)
    ...
    def forward(self, x):
        ...
        xd4 = torch.cat((xd2,xd3,xd4),1)
        xd4 = self.change_channel_final(xd4)
        xd4 = self.channel_att5(xd4)
        #深指导浅 #【l】
        ...
        if self.method == 'zhongji_loss':
            ...
            for i in range(len(out)):
                ...
                # print(((out[i].sum(dim=3)).sum(dim=2)).sum(dim=1),'from',i)
            # out[0] = self.channel_att2(out[0])
            # print((out[i].sum(dim=3)).sum(dim=2),'is',0)
            # out[1] = self.channel_att3(out[1])
            # print((out[i].sum(dim=3)).sum(dim=2),'is',1)
            # out[2] = self.channel_att4(out[2])
            # print((out[i].sum(dim=3)).sum(dim=2).sum,'is',2)
            return out[i]
