...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, bias=False)#缩
        self.bn1 = nn.BatchNorm2d(planes, momentum=BN_MOMENTUM)
...
class MultiResNet(nn.Module):
    ...
    def __init__(self, block, layers, cfg, **kwargs):  # **kwargs代表什么含义
        ...
        self.layer3_2 = self._make_layer(block, 256, layers[3], stride=1,new_branch=True)  #补长和通道不变
        self.deconv_layers3_2 = self._make_deconv_layer(
            extra.STAGE3.NUM_DECONV_LAYERS,  # 3
            extra.STAGE3.NUM_DECONV_FILTERS,  # 256 256 256
            extra.STAGE3.NUM_DECONV_KERNELS,  # 4 4 4
        )
        ...
        #     padding=0
        # )
        self.layer4 = self._make_layer(block, 512, layers[4], stride=2)  # 3
    ...
    def _make_layer(self, block, planes, blocks, stride=1,new_branch=False):
        downsample = None
        ...
        if new_branch == False:#如果是新分支就不变inplanes
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        x4 = self.layer4(x3_2)  # 前4层就是一般的resnet结构？：是
        # print(x.shape,'over')
        ...
        xd3 = self.deconv_layers3[0](x3)
        # print(len(self.deconv_layers3),xd3_first.shape,xd2.shape)
        xd3 = self.deconv_layers3[1](xd3)
        xd3_2 = self.deconv_layers3_2[0](x3_2)
        # print(len(self.deconv_layers3),xd3_2_first.shape,xd2.shape)
        xd3_2 = self.deconv_layers3_2[1](xd3_2)
    ...
    def init_weights(self, pretrained=''):
        if os.path.isfile(pretrained):
            ...
            logger.info('=> init deconv3_2 weights from normal distribution')
            for name, m in self.deconv_layers3_2.named_modules():
                if isinstance(m, nn.Conv2d):
                    # nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                    nn.init.normal_(m.weight, std=0.001)
                if isinstance(m, nn.ConvTranspose2d):
...
resnet_spec = {
    ...
    50: (Bottleneck, [3, 4, 3,3,3]),  # 【】和34的差别在？Bottleneck？
