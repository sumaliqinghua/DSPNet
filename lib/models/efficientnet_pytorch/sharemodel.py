...
class EfficientNet(nn.Module):
    ...
    def __init__(self, cfg,blocks_args=None, global_params=None,**kwargs):#不会被前面两个参数干扰了吧
        ...
        self. change_channel_2 = self._change_channel(extra.DECONV_INCHANNELS[0],128)
        # self. change_channel_3 = self._change_channel(extra.DECONV_INCHANNELS[1],256)
        # self. change_channel_4 = self._change_channel(extra.DECONV_INCHANNELS[2],256)
    ...
    def forward(self, inputs):
        ...
        xd2 = self.deconv_layers5[-1](x2)  #【】+xd4吗还是xd3y;用xd3_first吗不用的话就x2
        # xd2 = self.channel_att2(xd2)三
        xd3 = self.deconv_layers3[0](x3)#这儿是x3不是x2；加x2吗？
        xd3 = self.deconv_layers5[-1](xd3)#【y】+xd4吗
        # xd3 = self.channel_att3(xd3)
        xd4 = self.deconv_layers4[0](x4)#这儿是x3不是x2；加x2吗？
        xd4 = self.deconv_layers5[-1](xd4)#【y】+xd4吗
