...
class DwTrans(nn.Module):
    def __init__(self,in_channels,out_channels,kernel_size=3,stride=2,padding=1,output_padding=0,bias=False):
        ...
        self.conv_dw_3x3 = nn.Sequential(
        ...
        self.conv_1x1 = nn.Sequential(
            ...
            nn.ReLU(inplace=True))
        self.conv_dw_3x3_1 = nn.Sequential(
    ...
    def forward(self,x):
        x_ = self.conv_group(x)
        x = self.dw_spatial_att(x_)
        x = self.conv_dw_3x3(x)+x_#
        x_ = self.conv_1x1(x)
        x = self.dw_channel_att(x_)
        x = self.conv_dw_3x3_1(x_)+x#
        return x
