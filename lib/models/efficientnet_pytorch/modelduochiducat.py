...
64tongzhu ding
0.003
dw att zhuan duochidukongcatwei1 1x1 tong
'''
...
class SSEBlock(nn.Module):
    def __init__(self, inchannel):
        ...
        self.att_1 = nn.Conv2d(inchannel, 1, kernel_size=1,stride=1, padding=0, bias=False)
        self.att_3 = nn.Conv2d(inchannel, 1, kernel_size=3,stride=1, padding=1, bias=False)
        self.att_5 = nn.Conv2d(inchannel, 1, kernel_size=5,stride=1, padding=2, bias=False)
        self.conv1 = nn.Conv2d(3, 1, kernel_size=1,stride=1, padding=0, bias=False)

        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        # Returns a new tensor with the same data as the self tensor but of a different size.
        spa_se = torch.cat((self.att_1(x),self.att_3(x),self.att_5(x)),1)
        spa_se = self.sigmoid(self.conv1(spa_se))
