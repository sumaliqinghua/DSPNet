...
class SimpleSelfAttention(nn.Module):
    def __init__(self, n_in:int, ks=1, sym=False):#, n_out:int):
        ...
        self.convexp = nn.Conv2d(n_in, n_in, ks, padding=ks//2, bias=False)
    ...
    def forward(self,x):
        ...
        convx = self.convexp(x)   # (C,C) * (C,N) = (C,N)   => O(NC^2)
        # print(x.shape,x_.shape)
        xxT = torch.mm(x_,x_.permute(1,0).contiguous())   # (C,N) * (N,C) = (C,C)   => O(NC^2)
        o = torch.matmul(xxT, x)   # (C,C) * (C,N) = (C,N)   => O(NC^2)
        o = self.convexp(o)
        o = self.gamma * o + x
