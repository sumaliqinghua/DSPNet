...
4 fenzhi
tongzhu ding
dw att+res bili+kongzhu+tongzhu
'''
...
class DwTrans(nn.Module):
    def __init__(self,in_channels,out_channels,kernel_size=3,stride=2,padding=1,output_padding=0,bias=False):
        ...
        self.conv_1x1 = nn.Conv2d(in_channels,in_channels,kernel_size=1,bias=False)
        self.conv_1x1_final = nn.Conv2d(in_channels,out_channels,kernel_size=1,bias=False)
        self.dw_spatial_att = SSEBlock(in_channels)
        self.dw_channel_att = SEBlock(in_channels,2)
    def forward(self,x):
        x_ = self.dw_spatial_att(x)#fangzhizhongdianxinxidiushi
        x = torch.nn.functional.interpolate(x,scale_factor=2,mode='bilinear',align_corners=True)
        x = self.conv_group(x_)
        x_ = self.conv_1x1(x)
        x_ = self.dw_channel_att(x_)
        x_ += x
        x_ = self.conv_1x1_final(x_)
        return x_
