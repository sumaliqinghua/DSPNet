...
class Bottle2neck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, scale = 4):
        ...
        width = int((inplanes/scale))#先用in试试
...
# class MDTConv(nn.Module):
#     def __init__(self, channels, out_channels, dilation, stride):  # padding不传了 k_s变成4组列表
#         super(MDTConv, self).__init__()
#         self.num_groups = len(dilation)  # 【see】多卷积的列表[2,4,6,8]
#         self.split_channels = self._SplitChannels(
#             channels, self.num_groups)  # 通过组数将原通道数分为组数份，不足的放给0
#         self.mixed_depthwise_conv = nn.ModuleList([])
#         for i in range(self.num_groups):
#             self.mixed_depthwise_conv.append(nn.Conv2d(
#                 self.split_channels[i],
#                 self.split_channels[i],
#                 dilation=dilation[i],#现在传的是dilation参数
#                 kernel_size = 3,
#                 stride=stride,
#                 padding=dilation[i],  # padding计算公式得来
#                 groups=self.split_channels[i],  # 分组内也是可分离
#                 bias=False
#             ))
#         self.conv_1x1 = nn.Conv2d(
#             channels, out_channels, kernel_size=1, bias=False)
#         self.dw_spatial_att = SSEBlock(channels)
#         self.dw_channel_att = SEBlock(out_channels,2)
#     def _SplitChannels(self, channels, num_groups):
...
#         # print(x.shape)
#         x = torch.cat(x, dim=1)  # 【】后接bn relu吗
#         x = self.dw_spatial_att(x)
#         x = self.conv_1x1(x) #【c】顺序
#         x = self.dw_channel_att(x)
#         x = torch.nn.functional.interpolate(x,scale_factor=2,mode='bilinear',align_corners=True)#[]xianhou
...
class DwTrans(nn.Module):
    def __init__(self,in_channels,out_channels,kernel_size=3,stride=1,padding=1,bias=False):
        ...
        self.dw_spatial_att = SSEBlock(in_channels)
        self.dw_channel_att = SEBlock(out_channels,2)
        self.res2block = Bottle2neck(in_channels,out_channels)
    def forward(self,x):
        x_ = self.dw_spatial_att(x)
        # print('dw1',x.shape,type(x))
        # x = self.conv_1x1(x) #【c】顺序
        x_ = self.res2block(x_)
        # print('dw2',x.shape,type(x))
        x = self.dw_channel_att(x_)
        x = torch.nn.functional.interpolate(x,scale_factor=2,mode='bilinear',align_corners=True)#[]xianhou
        # print('dw3',x.shape,type(x))
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, cfg,blocks_args=None, global_params=None,**kwargs):#不会被前面两个参数干扰了吧
        ...
        self.deconv_laye3rs5 = self._make_deconv_layer(
    ...
    def extract_features(self, inputs):
        ...
        for idx, block in enumerate(self._blocks):
            ...
            #【see】
            # print(x.shape)
            if idx == self.outplane_idx[0]:#3 4 分辨率32
                x2 = x
            if idx == self.outplane_idx[1]:#5 10 16
                x3 = x
            if idx == self.outplane_idx[2]:#15 8
                x4 = x
            if idx == self.outplane_idx[3]:#15 8
                x5 = x
            #看看idx多长，是所有层数:15对
        # Head
        # x = relu_fn(self._bn1(self._conv_head(x)))【】这个要不要
        # print('idx is {}'.format(idx))2
        return x2,x3,x4,x5
        # Head
    ...
    def forward(self, inputs):
        ...
        # Convolution layers
        x2,x3,x4,x5 = self.extract_features(inputs)
        ...
        xd4 = self.deconv_layers4[0](x4)#这儿是x3不是x2；加x2吗？
        xd4 = self.deconv_layers4[1](xd4)#【y】+xd4吗
        xd4 = self.channel_att4(xd4)
        xd5 = self.deconv_layers5[0](x5)#
        xd5 = self.deconv_layers5[1](xd5)  #因为d5刚经过了relu值会比较小
        xd5 = self.deconv_layers5[2](xd5)#【】加x1吗
        xd5 = self.channel_att5(xd5)
    ...
    def init_weights(self, deconvpretrained=True):
        if deconvpretrained:#是否要对后面的deconv进行与训练
            ...
            logger.info('=> init deconv5 weights from normal distribution')
            for name, m in self.deconv_layers5.named_modules():  # 【】有没有简便一点的对几个层都处理的
                if isinstance(m, nn.Conv2d):
                    # nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                    nn.init.normal_(m.weight, std=0.001)
                if isinstance(m, nn.ConvTranspose2d):
