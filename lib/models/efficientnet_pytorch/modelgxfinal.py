    ...
    def forward(self, inputs):
        ...
        if self.method == 'zhongji_loss':
            ...
            # for i in range(len(out)):
                # print(out[i].shape)
            out[0] = self.final_layer_0(out[0])
            out[1] = self.final_layer_1(out[1])
            out[2] = self.final_layer_2(out[2])
            out[3] = self.final_layer_3(out[3])
        ...
        if deconvpretrained:#是否要对后面的deconv进行与训练
            ...
            for m in self.final_layer_1.modules():
                if isinstance(m, nn.Conv2d):
                    ...
                    nn.init.normal_(m.weight, std=0.001)
                    nn.init.constant_(m.bias, 0)
            for m in self.final_layer_2.modules():
                if isinstance(m, nn.Conv2d):
                    # nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                    logger.info(
                        '=> init {}.weight as normal(0, 0.001)'.format(name))
                    ...
                    nn.init.normal_(m.weight, std=0.001)
                    nn.init.constant_(m.bias, 0)
            for m in self.final_layer_3.modules():
                if isinstance(m, nn.Conv2d):
                    # nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                    logger.info(
                        '=> init {}.weight as normal(0, 0.001)'.format(name))
