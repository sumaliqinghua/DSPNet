...
class EfficientNet(nn.Module):
    ...
    def __init__(self, cfg,blocks_args=None, global_params=None,**kwargs):#不会被前面两个参数干扰了吧
        ...
        for block_args in self._blocks_args: #【c】把它拆成各个layer
#【c】打印_blocks_args长度:7，然后以block_args为整体写成layer1234
#或者直接改下面那儿，让它把遍历途中结果输出来
            ...
            # The first block needs to take care of stride and filter size increase.是指当前stage的第一个，同通道，可能不止一layer
            self._blocks.append(MBConvBlock(block_args, self._global_params))#第一个用给的通道数
            if block_args.num_repeat > 1:
                block_args = block_args._replace(input_filters=block_args.output_filters, stride=1)#后面的用前面的输出
            for _ in range(block_args.num_repeat - 1):#layers
    ...
    def extract_features(self, inputs):
        ...
        for idx, block in enumerate(self._blocks):
            ...
            if idx == 15:#15
                x4 = x
            #看看idx多长，是所有层数？还是stage数？if idx==指定idx输出x1 x2 x3留作上采样
        # Head
