...
class MDTConv(nn.Module):
    def __init__(self, channels, out_channels, kernel_size, stride):  # padding不传了 k_s变成4组列表
        super(MDTConv, self).__init__()
        self.num_groups = len(kernel_size)  # 【see】多卷积的列表[2,4,6,8]
        self.split_channels = self._SplitChannels(
            channels, self.num_groups)  # 通过组数将原通道数分为组数份，不足的放给0
        self.mixed_depthwise_conv = nn.ModuleList([])
        for i in range(self.num_groups):
            self.mixed_depthwise_conv.append(nn.ConvTranspose2d(
                self.split_channels[i],
                self.split_channels[i],
                kernel_size[i],
                stride=stride,
                padding=kernel_size[i] // 2-1,  # 【see】padding计算公式
                output_padding=kernel_size[i] % 2,#jishu 1
                groups=self.split_channels[i],  # 分组内也是可分离
                bias=False
            ))
        self.conv_1x1 = nn.Conv2d(
            channels, out_channels, kernel_size=1, bias=False)
        self.dw_spatial_att = SSEBlock(out_channels)
