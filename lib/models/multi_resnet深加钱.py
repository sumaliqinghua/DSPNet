...
class MultiResNet(nn.Module):
    ...
    def __init__(self, block, layers, cfg, **kwargs):  # **kwargs代表什么含义
        ...
        self. change_channel_2 = self._change_channel(512,256)#和eff有点不一样
        ...
        self.change_channel_3 =self._change_channel(1024,256)
    ...
    def _make_deconv_layer(self, num_layers, num_filters, num_kernels,in_channel):
        ...
        layers = []
        in_channels = in_channel#【see】从yaml传来的
    ...
    def forward(self, x):
        ...
        #替换 x2_ = self.relu(self.change_channel_2(x2))
        # x3_ =self.relu(self.change_channel_3(x3))#左

        # # xd4_first = self.deconv_layers4[0](x4)#这么写不是让xd3_first加了两次x3???
        # xd4_first = self.deconv_layers4[0](x4)#【c】改还没跑
        # # xd4_second = self.deconv_layers4[1](xd4_first)+x2_  #
        # xd4_second = self.deconv_layers4[1](xd4_first+x3_)  #
        # xd4 = self.deconv_layers4[2](xd4_second+x2_)#【】加x1吗
        # # xd3_first = self.deconv_layers3[0](x3_+xd4_first)#用x3_吗，不用的话就deconv+4second；但是这样好像能减小参数
        # xd3_first = self.deconv_layers3[0](x3_+xd4_first)#用x3_吗，不用的话就deconv+4second；但是这样好像能减小参数；加x2吗？
        # xd3 = self.deconv_layers3[1](xd3_first+xd4_second)+xd4 #【】+xd4吗
        # xd2 = self.deconv_layers2[0](x2_+xd3_first)+xd3  #【】+xd4吗还是xd3y;用xd3_first吗不用的话就x2
        #替换

        x2_ = self.relu(self.change_channel_2(x2))
        x3_ =self.relu(self.change_channel_3(x3))#deconv3用了之后再改通道
        xd4 = self.deconv_layers4[0](x4)+x3_
        xd4 = self.deconv_layers4[1](xd4)+x2_  # x2还是xd3_first
        xd4 = self.deconv_layers4[2](xd4)#还是xd3??

        xd3_first = self.deconv_layers3[0](x3)+x2_
        # print(len(self.deconv_layers3),xd3_first.shape,xd2.shape)
        xd3 = self.deconv_layers3[1](xd3_first)+xd4
        xd2 = self.deconv_layers2[0](x2)+xd4  # 跳接+x1
