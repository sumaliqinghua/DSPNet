...
class DwTrans(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=2, padding=1, output_padding=0, bias=False):
        ...
        self.conv_1x1in = nn.Sequential(
            nn.Conv2d(in_channels, 6*in_channels, kernel_size=1, bias=False),
            nn.BatchNorm2d(6*in_channels,momentum=BN_MOMENTUM),
        ...
        self.conv_group = nn.ConvTranspose2d(
            ...
            groups=6*in_channels,
            bias=False
        )
        self.conv_1x1out = nn.Sequential(
            nn.Conv2d(6*in_channels, out_channels, kernel_size=1, bias=False),
...
class MultiResNet(nn.Module):
    ...
    def __init__(self, block, layers, cfg, **kwargs):  # **kwargs代表什么含义
        ...
        self.change_channel2=self._change_channel(512,64)
        self.change_channel3=self._change_channel(1024,128)
        self.change_channel4=self._change_channel(2048,256)
        # self.sse_attention = SSEBlock(256)
        # used for deconv layers

        self.final_layer = nn.Conv2d(
            in_channels=64,  # 【】把deconv改为resnet对应的通道数
    ...
    def forward(self, x):
        ...
        x2 = self.change_channel2(x2)
        xd2 = self.deconv_layers2[0](x2)  # 跳接+x1

        x3 = self.change_channel3(x3)
